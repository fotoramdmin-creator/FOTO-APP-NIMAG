...
const fmtDia = (dia: string | Date): string => {
  if (!dia) return "";
  try {
    const d =
      dia instanceof Date
        ? dia
        : new Date(`${String(dia).slice(0, 10)}T00:00:00`);
...
const loadImageInfo = (src: string): Promise<{ w: number; h: number }> =>
...
function buildLayout(
  { dia, entradas, ingresos, salidas, neto, caja, aRetirar }: TicketParams,
  doc: jsPDF
) {
  const PAGE_W = 58;
  const M = 4;
  ...
  const titleLines: string[] = doc.splitTextToSize("CORTE DE CAJA", contentW);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(SMALL_SIZE);
  const diaLabel = fmtDia(dia);
  const diaLines: string[] = doc.splitTextToSize(
...
function computeNeededHeight(
  ...
  logoDimsMm: { w: number; h: number } | null
) {
  const {
    M,
    LH,
...
function makeLeaderDots(doc: jsPDF, availableMm: number) {
